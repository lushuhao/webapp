...
def select(sql, args, size=None):
	...
	with (yield from __pool) as conn:
		# yield from 调用子协程并直接获得子协程的返回结果
		cur = yield from conn.cursor(aiomysql.DictCursor)
		# SQL语句的占位符是？，MySQL的占位符是%s，select()函数在内部自动替换
		yield from cur.excute(sql.replace('?', '%'), args or ())
		if size:
			# 传入size参数，就获取最多指定数量的记录
...
class IntegerField(Field):
	...
	def __init__(self, name=None, primary_key=False, default=0):
		super().__init__(name, 'bigit', primary_key, default)
...
class ModelMetaclass(type):
	...
	def __new__(cls, name, bases, attrs):
		...
		for k, v in attrs.items():    # 获取class的所有属性
			if isinstance(v, Field):    # 判断类型，v是不是数据库数据类型
